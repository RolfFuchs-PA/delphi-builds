...
emit("}")
emit("$script:VaultCheckedOut = $false  # Tracks whether Vault CheckOut was performed (used to guard UndoCheckOut on error)")
emit("")
...
emit("    $script:VaultCheckedOut = $true")
emit("}")
emit("")
...
emit("    $auth = Get-VaultAuthArgs")
emit("    & $script:VaultExe get @auth -repository $Repository \"$Path\"")
